...
`echo "text" > file` - used to write output to a file, overwriting its contents

`echo "text" >> file` - used to append output to a file

`stdout` - standard output of a command or program

`stderr` - standard error output of a command or program

`crontab -e` - used to edit the current user's cron jobs

`crontab -l` - used to display the current user's cron jobs

`systemctl daemon-reload` - used to reload systemd unit configuration files

`systemctl start` - used to start a systemd service

`systemctl enable --now` - used to enable and immediately start a systemd unit

`systemctl status` - used to check the status of a systemd service or timer

`systemctl stop` - used to stop a systemd service or timer

`systemd service` - defines how a program or task should be executed and managed by systemd

`systemd timer` - used to schedule a systemd service to run automatically

`Type=oneshot` - used for a systemd service that performs a task and then exits

`ExecStart=` - defines the command that systemd should execute

`StandardOutput=append:` - used to append standard output to a specified file

`StandardError=append:` - used to append standard error output to a specified file

`journalctl -u` - used to view logs for a specific systemd service

`journalctl -u service -n 5` - used to display the latest 5 log entries for a specific service

`systemctl list-timers` - used to display systemd timers and their schedules
